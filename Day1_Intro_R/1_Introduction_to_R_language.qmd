...
-   Jupyter notebook (.ipynb)
    ...
    -   You need to install the R kernel for Jupyter notebook use the [following tutorial](https://github.com/IRkernel/IRkernel) or you can try the [ark kernel](https://github.com/posit-dev/ark).
...
To **print out results**, simply write print() with parenthesis, but it is not compulsory in R to output a result.
...
**Curly brackets**: Like many programming language, R does not care about the spaces because it is always bound to brackets (parentheses, brackets or curly brackets). Indentation is only for aesthetic matter and user are free to indent their code following their taste (unlike Python or Nim that can have indentation error). For example the indentation of the second `print()` does not break the code since it is in the brackets:
...
-   Code is run inside an environment (R version, project and environment). It can always be stop by pressing the "Stop" button or Ctrl+C shortcut (inside quarto/rmarkdown notebook or a script).
...
Tips: You can generate a continuous vector between two values using the signe `:` as follow:
...
-   If it does not works you can either:
    ...
    -   Change the code to `df <- read_csv("data.csv1")`

```{r}
df <- read_csv("Day1_Intro_R/data.csv")
```
